...
class Bin:
    ...
    def get_volume(self):
        return set_to_decimal(
            self.length * self.height * self.width, self.number_of_decimals * 3
        )
    ...
    def get_filling_ratio(self):
        ...
        try:
            ...
            log.warning(
                "failed to calculate filling ratio on %s, volume: %s: %s",
                self,
                total_filling_volume,
                err,
            )
            raise ValueError("failed to calculate filling ratio") from err
        else:
            return set_to_decimal(total_filling_ratio, self.number_of_decimals * 3)
    ...
    def can_hold_item_with_rotation(self, item, pivot):
        ...
        for i in range(0, len(RotationType.ALL)):
            ...
            if (
                pivot[0] + dimension[0] <= self.length
                and pivot[1] + dimension[1] <= self.width  # x-axis
                and pivot[2] + dimension[2] <= self.height  # y-axis  # z-axis
            ):
                ...
                if fit:
                    if (
                        self.get_total_weight() + item.weight > self.capacity
                    ):  # estimate whether bin reaches its capacity
                        fit = False
                        item.position = [0, 0, 0]
                        continue
        ...
        if not rotation_type_list:
            ...
            if rotation_type_number == 1:
                ...
                if n == 1:
                    ...
                    p = 0
                    while p < len(margins_3d_list_temp):
                        if margins_3d_list_temp[p][0] == min(
                            margin_3d
                        ) and margins_3d_list_temp[p][1] == min(margin_2d):
                            m += 1
                ...
                if m == 1:
                    ...
                    while p < len(margins_3d_list_temp):
                        if (
                            margins_3d_list_temp[p][0] == min(margin_3d)
                            and margins_3d_list_temp[p][1] == min(margin_2d)
                            and margins_3d_list_temp[p][2] == min(margin_1d)
                        ):
                            item.rotation_type = rotation_type_list[p]
                            self.items.append(item)
                            self.total_items += 1
    ...
    def string(self):
        return (
            "%s(%sx%sx%s, max_weight:%s) vol(%s) item_number(%s) filling_ratio(%s)"
            % (
                self.size,
                self.length,
                self.width,
