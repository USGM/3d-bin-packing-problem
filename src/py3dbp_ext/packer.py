...
from .constants import Axis, START_POSITION, DEFAULT_NUMBER_OF_DECIMALS
...
class Packer:
        ...
        for axis in range(0, 3):
            ...
            while n < len(items_in_bin):
                ...
                if axis == Axis.LENGTH:  # axis = 0/ x-axis
                    ib = items_in_bin[n]
                    pivot = [
                        ib.position[0] + ib.get_dimension()[0],
                        ib.position[1],
                        ib.position[2],
                    ]
                    ...
                    if try_put_item:
                        ...
                        while q < len(items_in_bin_temp):
                            if items_in_bin_temp[q] == items_in_bin[n]:
                                ...
                                if (
                                    ib_neighbor.position[0]
                                    > ib.position[0] + ib.get_dimension()[0]
                                    and ib_neighbor.position[1]
                                    + ib_neighbor.get_dimension()[1]
                                    > ib.position[1]
                                    and ib_neighbor.position[2]
                                    + ib_neighbor.get_dimension()[2]
                                    > ib.position[2]
                                ):
                                    right_neighbor = True
                                    x_distance = ib_neighbor.position[0] - (
                                        ib.position[0] + ib.get_dimension()[0]
                                    )
                                    ib_neigh_x_axis.append(x_distance)

                                elif (
                                    ib_neighbor.position[1]
                                    >= ib.position[1] + ib.get_dimension()[1]
                                    and ib_neighbor.position[0]
                                    + ib_neighbor.get_dimension()[0]
                                    > ib.position[0] + ib.get_dimension()[0]
                                    and ib_neighbor.position[2]
                                    + ib_neighbor.get_dimension()[2]
                                    > ib.position[2]
                                    ...
                                    front_neighbor = True
                                    y_distance = (
                                        ib_neighbor.position[1] - ib.position[1]
                                    )
                                    ib_neigh_y_axis.append(y_distance)

                                elif (
                                    ib_neighbor.position[2]
                                    ...
                                    above_neighbor = True
                                    z_distance = (
                                        ib_neighbor.position[2] - ib.position[2]
                                    )
                        ...
                        while q < len(items_in_bin_temp):
                            if items_in_bin_temp[q] == items_in_bin[n]:
                                ...
                                if (
                                    ...
                                    right_neighbor = True
                                    x_distance = (
                                        ib_neighbor.position[0] - ib.position[0]
                                    )
                                    ib_neigh_x_axis.append(x_distance)
                                    ...
                                    front_neighbor = True
                                    y_distance = ib_neighbor.position[1] - (
                                        ib.position[1] + ib.get_dimension()[1]
                                    )
                                    ib_neigh_y_axis.append(y_distance)
    ...
    def choose_pivot_point(self, bin, item):
        ...
        if pivot_available:
            ...
            if n == 1:
                ...
                p = 0
                while p < len(pivot_available_temp):
                    if pivot_available_temp[p][0] == min(
                        pivot_available_temp[p]
                    ) and pivot_available_temp[p][1] == min(vertex_2d):
                        m += 1
            ...
            if m == 1:
                ...
                while p < len(pivot_available_temp):
                    if (
                        pivot_available_temp[p][0] == min(pivot_available_temp[p])
                        and pivot_available_temp[p][1] == min(vertex_2d)
                        and pivot_available_temp[p][2] == min(vertex_1d)
                    ):
                        return pivot_available[p]
    ...
    def pack_to_bin(self, bin, item):
        ...
        if not bin.items:
            response = bin.put_item(
                item, START_POSITION, [bin.length, bin.width, bin.height]
            )
    ...
    def pack(self, bigger_first=True, number_of_decimals=DEFAULT_NUMBER_OF_DECIMALS):
        ...
        Returns:
            the optimal bin with highest packing rate.
        """
        ...
        self.bins.sort(
            key=lambda bin: bin.get_volume()
        )  # default order of bins: from smallest to biggest
        self.unplaced_items.sort(
            key=lambda unplaced_item: unplaced_item.get_volume(), reverse=bigger_first
        )  # default order of items: from biggest to smallest
        ...
        for bin in self.bins:
            ...
            log.debug("BIN ::::::::::: %s", bin)
            log.debug("FITTED ITEMS:")
            for item in bin.items:
                log.debug("====> %s", item)

            log.debug("UNFITTED ITEMS:")
        ...
        for bin in self.bins:
            if bin.get_filling_ratio() == max_filling_ratio:
                ...
                log.debug("Selected bin with highest filling ratio: %s", bin)
                best_bin = bin
